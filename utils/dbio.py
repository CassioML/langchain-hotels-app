...
def find_hotels_by_country_city(
    session: Session, keyspace: str, country: str, city: str
) -> List[Hotel]:
    stmt_str = SELECT_HOTEL_BY_COUNTRY_CITY_CQL.format(
        keyspace=keyspace, table=HOTELS_TABLE_NAME
    )
    stmt = _get_prepared(session, stmt_str)
    result_set = session.execute(stmt, (country, city))
    return [Hotel(**result._asdict()) for result in result_set]
